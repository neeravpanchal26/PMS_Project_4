import { Injectable, EventEmitter, Component, ViewEncapsulation, ChangeDetectorRef, Inject, PLATFORM_ID, ViewChild, Input, Output, HostListener, NgModule } from '@angular/core';
...
class SliderLightboxComponent {
    ...
    set currentImageSrc(url) {
        if (url && typeof (url) === 'string') {
            this.fileExtension = url.replace(/^.*\./, '');
            if (this.imageSliderService.base64FileExtension(url)
                && (validFileExtensions$2.indexOf(this.imageSliderService.base64FileExtension(url).toLowerCase()) > -1
                    || validVideoExtensions$2.indexOf(this.imageSliderService.base64FileExtension(url).toLowerCase()) > -1)) {
                this.fileExtension = this.imageSliderService.base64FileExtension(url);
            }
            // verify for youtube url
            /** @type {?} */
            const match = url.match(youtubeRegExp$2);
            if (match && match[2].length === 11) {
                this.type = '';
                setTimeout((/**
                 * @return {?}
            ...
            else if (this.fileExtension && validFileExtensions$2.indexOf(this.fileExtension.toLowerCase()) > -1) {
                this.type = this.IMAGE;
                this.fileUrl = this.sanitizer.bypassSecurityTrustResourceUrl(url);
            }
            else if (this.fileExtension && validVideoExtensions$2.indexOf(this.fileExtension.toLowerCase()) > -1) {
                this.type = '';
                setTimeout((/**
                 * @return {?}
                 */
